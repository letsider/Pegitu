...
function getMessages() {
	...
	$.getJSON('chat/get-message.php?dateConnexion='+$("#dateConnexion").val(), function(data) {
			...
			if(data['error'] == '0') {
				...
				$("#text").html('');
				$(location).attr('href',"chat/chat.php");
			}
	});
}
...
function getOnlineUsers() {
	// On lance la requête ajax
	$.getJSON('chat/get-online.php', function(data) {
		...
		if(data['error'] == '0') {		
			...
			for (var id in data['list']) {
				...
				online += '<img src="chat/status-'+image+'.gif" /> ';
